...
DEFAULT_SPLIT_DIR = Path("Ophthalmic_Scans") / "splits" / "tumor_and_fluid_segmentation_oct2"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Review OCT tumor/fluid masks from a split and save ok/not_ok into local metadata JSON files."
    )
    parser.add_argument(
        "--split-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Start review from this zero-based record index after filtering to raw entries.",
    )
...
def metadata_path_for_image(repo_root: Path, image_path: str) -> Path:
    ...
    if image_rel.parts[0] != "raw":
        raise ValueError(f"Expected raw image path, got: {image_path}")
    metadata_rel = Path(*image_rel.parts[:-2]) / "metadata" / f"{image_rel.stem}.json"
    return repo_root / "Ophthalmic_Scans" / metadata_rel


def load_split_records(repo_root: Path, split_dir: Path) -> list[ReviewRecord]:
    ...
    for split_name in ("train", "val", "test"):
        ...
        raw_df = df[df["image_path"].astype(str).str.startswith(RAW_PREFIX)].copy()
    ...
    if not records:
        raise ValueError(f"No raw records found in split directory: {split_dir}")
    return records
...
class ReviewApp:
    def __init__(self, root: tk.Tk, records: list[ReviewRecord], start_index: int):
        ...
        self.root.title("OCT Mask Review")
    ...
    def refresh(self) -> None:
        ...
        self.status_var.set(f"Record {self.index + 1}/{len(self.records)} | split: {record.split_name}")
        self.path_var.set(
            "\n".join(
                [
...
def main() -> None:
    ...
    records = load_split_records(repo_root, split_dir)
